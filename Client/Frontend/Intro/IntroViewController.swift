...
private let IntroViewControllerSeenProfileKey = "IntroViewControllerSeen"
...
class IntroViewController: UIViewController, UIScrollViewDelegate {
    ...
    /// Initialize the on-boarding controller. Or return nil if the user has already seen
    /// it. Also marks the controller as seen so that this logic is in one place.
    convenience init?(profile: Profile, reset: Bool = false) {
        self.init()
        if !reset {
            if let hasSeenIntro = profile.prefs.intForKey(IntroViewControllerSeenProfileKey) {
                return nil
            }
        }
